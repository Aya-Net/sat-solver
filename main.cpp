...
void fun_1() {
    ...
    constexpr bool output_detail = true;
    ...
    for (int i = 7; i <= 7; i++) {
        ...
        if (res) {
            ...
            for (int j = 1; j <= cnf->num_vars(); j++) {
                std::cout << (int) sat_solver.decisions()[j] << " ";
            }
            std::cout << std::endl;
            if (output_detail)
                std::cout << (result.type == CONFLICT ? "Incorrect" : "Satisfied");
        } else {
            if (output_detail)
        ...
        if (output_detail)
            std::cout << ", read time: " << (double) read_time / 1000
